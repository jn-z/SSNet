...
class Creat_DlaNet(nn.Module):
        ...
        for head in self.heads:
            ...
            if head_conv > 0:  # and {'cl_id','con_id'} not in head:
                if 'hm' in head:
                    ...
                    self.feature_add = nn.Conv2d(256, classes, kernel_size=1, stride=1, padding=0, bias=True)
                else:
                    ...
                    fc = nn.Sequential(
                        ...
                        nn.Conv2d(head_conv, classes,
                                  ...
                                  padding=0, bias=True))
                if 'hm' in head:
                    self.feature_add.bias.data.fill_(-2.19)
                else:
                    fill_fc_weights(fc)
            else:
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            if 'hm' in head:
                #pdb.set_trace()
                feature_layer = self.__getattr__(head)(x)  # torch.Size([2, 256, 256, 256])
                mean_x_4 = torch.mean(feature_layer, dim=3, keepdim=True), # torch.Size([2, 256, 256, 1])
                var_x_4 = torch.var(feature_layer, dim=3, keepdim=True), # torch.Size([2, 256, 256, 1])
                #feature_layer = torch.cat((mean_x_4[0], torch.sqrt(var_x_4[0] + 0.00001)), dim=3) # torch.Size([2, 256, 256, 2])
                # std_x_5=torch.std(x_5,dim=1,unbiased=False)
                var_x = var_x_4[0]
                feature_layer = (feature_layer + mean_x_4[0]) * (var_x.expand(var_x.size(0),var_x.size(1),var_x.size(2),feature_layer.size(3)))
                ret[head] = self.feature_add(feature_layer)
                res.append(self.feature_add(feature_layer))
            else:
...
def DlaNet(num_layers=34, heads={'hm': 3, 'wh': 2, 'reg': 2}, head_conv=256, plot=False):
